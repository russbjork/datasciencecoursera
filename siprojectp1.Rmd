...
Based on plot1 and plot2:  
Theoritical Mean vs Sample Mean : Although the two figures are really different, we can notice that both means are very similar (around 5) : the first figure gives a   histogram of frequencies, and the second a histogram of densities. Furthermore, in Figure 2, the repartition of values is not the same on both sides of the mean, but the   mean is very similar.  

# Sample Variance versus Theoretical Variance:
Based on plot1 and plot2:  
Theoritical variance vs Sample Variance : We can notice that the larger n is, the more the variance of the mean distribution is low, that means that the sample is more   concentrated arouns the mean. This can be easily deducted from figures 1 and 2. In figure 1, we have nn = 1, and the dispersion of the values is very large, whereas in   figure 2 (nn = 40), the dispersion is much lower.  

# Distribution: 
Sample distribution of mean of 40 exponential values  

Below we draw a distibution of 1000 means of random exponential values, each mean built on 40 random exponential values.   
...
hist(mns, main ="Plot 2 : Histogram of 1000 means of 40 exponentials", xlab="Y",freq=FALSE) # histogram of the density 
